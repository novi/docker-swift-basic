FROM yusukeito/swift:release-3.1.1
...
RUN apt-get update && \
    apt-get install -y libxml2-dev libmariadbclient-dev git automake libtool autoconf uuid-dev libssl-dev libz-dev unzip libcurl4-gnutls-dev tzdata && \
    apt-get clean && \
